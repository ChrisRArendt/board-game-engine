<script lang="ts">
	...
	import { emit, activeSocketId } from '$lib/stores/network';
	...
	onMount(() => {
		registerGameEmit((type, data) => {
			const sid = get(activeSocketId);
			if (type === 'piece_select') emit(type, { ...data, oursocketid: sid });
			else emit(type, data);
		});

		fetch('/data/bsg_1/pieces.json')
			.then((r) => r.json())
			.then((j: GameDataJson) => {
				g.loadGameData(j);
				g.centerCamToVP();
...
<Toolbar
	...
	onSettings={() => (winSettings = true)}
	onConnection={() => (winConn = true)}
/>
